export declare function eventlakeClient(opts: EventlakeOptions): EventlakeClient
...
export type EventlakeClient = {
  ...
  /**
   * Similar to track(), but send unstructured payload to EventLake. No
...
/**
 * Type of eventlake function which is exported to window.eventlake when tracker is embedded from server
 */
export type EventlakeFunction = (action: 'track' | 'identify' | 'set', eventType: string, payload?: EventPayload) => void;
 ...
 */
export type IdentifyMethod = 'cookie' | 'ls'

/**
 * Policy configuration affects cookies storage and IP handling.
 *  - strict: EventLake doesn't store cookies and replaces last octet in IP address (10.10.10.10 -> 10.10.10.1)
 *  - keep: EventLake uses cookies for user identification and saves full IP
 *  - comply: EventLake checks customer country and tries to comply with Regulation laws (such as GDPR, UK's GDPR (PECR) and California's GDPR (CCPA)
 */
export type Policy = 'strict' | 'keep' | 'comply'

/**
 * Configuration options of EventLake
 */
export type EventlakeOptions = {

  /**
   * A custom fetch implementation. Here's how EventLake decides what functions to use to execute HTTP requests
   *
   * - If EventLake runs in browser, this parameter will be ignored. The best available API (most likely, XMLHttpRequest)
   *   will be used for sending reqs
   * - For node EventLake will use this param. If it's not set, EventLake will try to search for fetch in global environment
  ...
  /**
   * Forces EventLake SDK to use the fetch implementation (custom or default) even in browser
   */
  force_use_fetch?: any,
  ...
  /**
   * Send events to console only. Useful for local debugging.
   */
  console_transport?: boolean

  /**
   * API calls to send. (optional)
   */
  api_host?: string

  /**
   * Name of id cookie. __eventlake_id by default
   */
  cookie_name?: string
  /**
  ...
  key: string
  /**
   * If google analytics events should be intercepted. 
   * @default false
   */
  ga_interceptor?: boolean
  /**
   * If google analytics events should be intercepted.
   * @default false
   */
  segment_interceptor?: boolean
  /**
   * If URL of API server call should be obfuscate to by-pass adblockers
   *
   * @default false
   */
  ...
  /**
   * If EventLake should capture third-party cookies: either array
  ...
  /**
   * See comment on IdentifyMethod. Currently only 'cookie' and 'cookie-less' are supported
   */
  ...
  //NOTE: If any property is added here, please make sure it's added to browser.ts eventlakeProps as well
};
...
export type TrackingEnvironment = {
  ...
  /**
   * Returns source ip. If IP should be resolved by EventLake server, this method should return undefined
   */
...
/**
 * List of environments where EventLake tracker can work. See TrackingEnvironment above
...
export type EventPayload = Partial<ClientProperties> & {
   ...
   * request/response. In this case EventLake will try to use it for
...
export type EventSrc =
  'eventlake'    |                     //event came directly from EventLake
